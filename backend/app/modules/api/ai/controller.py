...
from backend.app.common.response import StreamResponse, SuccessResponse, ErrorResponse
from backend.app.modules.api.ai.schema import ChatQuerySchema, Chat, ChatItem
from backend.app.modules.api.ai.schema import ChatListResponse, ChatListResponseItem, ChatMessageListResponse, ChatMessageResponseItem, BasicResponse
...
async def chat(
    ...
    return StreamResponse(
        AIService.chat_stream_generator(query),
...
@AIRouter.get("/{session_id}/messages", summary="获取会话的消息历史", response_model=Union[ChatMessageListResponse, BasicResponse])
async def get_chat_messages(
    ...
    try:
        ...
        if not chat or chat.is_deleted or str(chat.user_id) != str(current_user.id):
            return BasicResponse(success=False, message="会话不存在或无权限")
        ...
        return BasicResponse(success=False, message=str(e))


@AIRouter.delete("/{session_id}", summary="删除聊天会话", response_model=BasicResponse)
async def delete_chat(
    ...
    return BasicResponse(success=success, message=msg)
